...
const RESOURCES: RefItem[] = [
  ...
  {
    no: "09",
    title: "Anthropic 工程博客：Building Effective Agents（Workflow 与 Agent 的界定）",
...
export default function ChapterReferences() {
  return (
    <section aria-label="第六章 参考来源" className="mt-24">
      <ChapterHeading id="references" index={6} title="参考来源" />
      <p className="mb-10 text-body text-text-secondary">
        本章全部事实性表述均可公开验证。以下论文与官方文档是深入学习的首选入口。
      </p>
      <div className="space-y-12">
        <RefGroup icon={<FileText size={18} />} title="论文" items={PAPERS} />
        <RefGroup icon={<BookOpen size={18} />} title="官方文档与资源" items={RESOURCES} delay={0.1} />
      </div>
    </section>
  );
}
